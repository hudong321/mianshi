    ...
    <van-row>
      ...
      <van-col span="19">
        ...
        <van-collapse v-show="active == 6" v-model="activeName" accordion>
          ...
          <van-collapse-item title="哪些场景下会出现OOM的情况" name="11">
                java中oom包括堆内存不足，方法区/元空间内存不足，线程内存不足，栈溢出等等。<br>
                1.堆内存溢出；一次性加载超大文件进入内存。生成对象过多无法释放。（jvm设置-Xmx -Xms，业务逻辑分批次处理，读取文件使用缓存，避免一次性读取）<br>
                2.方法区/元空间内存溢出；动态代理类生成对象过多，无法释放。<br>
                3.线程内存溢出；线程过多，无法释放。<br>
                4.栈内存溢出；无限递归调用。<br>
          </van-collapse-item>
        </van-collapse>
      </van-col>
